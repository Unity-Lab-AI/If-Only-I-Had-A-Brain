...
verified-scope: |
  ...
  unrelated lines).
  ADDED 2026-08-31: the fresh-walk milestone banner. The road no longer ends
  at a press — the press happened, and the milestone it delivered is that the
  three-tier memory path this roadmap has listed as shipped since May executed
  for the first time. Numbers (pin 15,082,717; tier1 4 / merged 21 /
  promoted 4; roster 9 + 11 upcoming; corpus 89/89 cells) were read from the
  live brain, not recalled. ⚠ The next milestone, the first cell pass, is an
  8-14 h CONSTRUCTION with no precedent — no cell has ever completed here.
last-verified: "f06ea30e 2026-08-31"
---

# ROADMAP — IF ONLY I HAD A BRAIN

## ⛔ START HERE — the plan begins at [§ Project Vision](#project-vision)

| If you want | Go to |
|---|---|
| Why this exists at all | [§ Project Vision](#project-vision) |
| Where she actually is today | [§ Current Status](#current-status) — **re-measured, not recalled** |
| The build phases and what each delivered | the phase sections below |
| What depends on what | [§ Dependency Graph](#dependency-graph) |
| What could still go wrong | [§ Risk Assessment](#risk-assessment) · [§ Decision Log](#decision-log) |

⚠ **THIS IS A PLANNING PAGE, AND THAT CHANGES WHAT "WRONG" MEANS HERE.** When it drifts from the code, the honest response is usually **"re-price the plan"** rather than *"a claim is false"* — a roadmap is allowed to describe intent. ⛔ **The exceptions are its status markers**, and those had rotted badly: two phases were headed **"IN PROGRESS"** for work that shipped in April, one of them for work that was not just finished but *deleted and replaced*, and the status table carried a **"Current date" of 2026-04-21** for nearly five months. **A roadmap that never marks a phase finished stops being a plan and becomes a list of things that look unfinished.**
...
> ⚠ **HISTORY, NOT THE PLAN.** These entries record what the road looked like on the day each was written. **They are deliberately not rewritten** — a roadmap's value includes being able to see what was expected and compare it against what happened, and editing them destroys exactly that.
>
> ⛔ **So read them as dated intent, never as current status.** Where an entry disagrees with [§ Current Status](#current-status), that section was **measured** and wins. They also carry internal milestone identifiers and branch names that nothing in the project resolves; those are tolerated here, in labelled history, and are out of place everywhere else on this page.

> Last updated: 2026-08-31 — **THE PRESS HAPPENED, AND THE WALK IS UNDER WAY ON A BRAIN THAT CAN FINALLY CONSOLIDATE WHAT IT LEARNS.** The fresh walk was pressed **07:23Z on `b1a5eb01`** and the boot verified green at 6.4 minutes: genuine wipe (no cell passed, every grade back to pre-K), the language-cortex geometry **pin HELD at 15,082,717** rather than the smaller size a RAM-only derive would have produced on that boot, 30 identity anchors preserved across the wipe, donor A40 at 10.18 Gn/s with zero upload failures, event-loop lag 0 ms. ⛔ **The milestone is not the press — it is that a system this roadmap has listed as shipped since May was running for the first time.** The three-tier episodic → consolidation → schema path had produced **nothing** on any boot in the project's history, because two of the three writers into the episodic store were gated on a flag that is true for the whole multi-week walk, and the third fires only on phase completion while phases were not completing. **Interleaved replay — the thing that separates similar representations from each other — had never executed**, so the waking repetition dose was quietly doing its job as well. Verified end to end at 18 minutes: four episodes, twenty-one folded by the exact-text merge, four promotions, and two named Tier-2 schemas. ⭐ **This changes what the remaining road costs**, because the standing plan to re-price the teach reps downward was explicitly conditioned on replay being proven real first — it now is, and that measurement can be taken on this walk rather than argued about. **Also landing in this press:** the training card publishes the **declared** roster (9 courses at kindergarten plus 11 more upcoming) instead of only the courses that had already run, and the academic prose corpus reached **89/89 cells · 874/874 topics · 12,075 sentences** — the computer-science track had held **zero** corpus across all eight of its K-12 grades. ⏱ **Next milestone is the first cell pass**, a constructed estimate of 8-14 h with no precedent to check it against: ⛔ **no cell has ever completed in this project's history.** The falsifier is written down — not passed by ~24 h means a heavy phase is not applying the rep/learning-rate compensation, and the console line that names it is its own absence.
>
...
>
> Last updated: 2026-08-21 — **ALL TRAINING RUNS ON THE DONOR GPUS; THE CPU IS THE COORDINATOR.** The operator's law made structural in one day and PROVEN by a full plasticity call-site audit: every vocab lane (all cells, all grades, all phases), the intra Oja pass, the alphabet drills, the sem→sem definition write, and lateral inhibition now dispatch to the donor — via the existing choke points, the already-deployed `hebbian_ranges` range-form verb, and a NEW donor binary (**donor-v0.3.26**, SPRS type 13 **masked bound plasticity**: pre reads the resident bound spikes at zero wire cost, post ships as a sparse row mask scattered device-side — the pre≠post shape that kept lateral inhibition on the CPU at a measured 31.8s per 60s gate window). On every converted lane the GPU carries full mass and the CPU keeps a sampled shadow for saves and probes; whenever a donor can't take a frame the CPU pass runs in full — nothing is ever dropped, old donors are byte-identical. The teach lane can no longer lie or silently drop (`throughput.boundHebbian` counts every stage; the batch cap flushes instead of dropping), gate vocab is spaced-repetition off a persisted receipt (hours → minutes, read live), every gate/probe propagate across ALL grades is chunked (the 3-4s loop slabs are gone), and the "4MB/s uplink" was exposed as the PUMP (≤14MB in flight) — native donors now keep 96MB in flight and every upload prints its measured MB/s. The donor pod self-updates from `releases/latest`; the `donor-v*` tag pipeline builds, attaches, bumps and publishes hands-off. **Remaining on this road:** the two signed-magnitude plain-Hebbian writes (predictive-error delta, WM encode) need a float-mask donor kernel (0.3.27 scope); worker-thread CPU-shadow math stays the wall-clock endgame for what little the coordinator still computes.
>
> Last updated: 2026-08-25 — **HER ART IS NOW A SKILL THAT IMPROVES, AND FOUR INSTRUMENTS STOPPED REPORTING HEALTH THEY COULD NOT KNOW.** No equation, weight format or geometry changed, so a Savestart stays format-legal — but the roadmap position moves on one axis. ⭐ **`budgetMul`:** how much of a remembered thing she commits to the page is a **trained** parameter now, in the same practice loop as jitter/underA/traceW/keepP/detailMul and scored by the same objective (cosine against her banked percept of the real thing) — so a more practised hand draws more of what it knows, and only because that measurably improved the resemblance. Measured: the doodle hand goes **101 strokes untrained → 235 practiced**. ⛔ **It was a CEILING before, and the ceiling was mine:** stroke budgets sized as a noise defence survived after the noise was fixed at its source, and Gee named it — *"cant make a art work in only 20 strokes it should increase in ability as she learns in art and stuff"*. **When a defence becomes unnecessary, its cost stops being free** — that is the roadmap lesson, not the number. The rest of the batch is honesty rather than reach: producer/consumer parity on the endocrine and introspection boards (**13/13** and **7/7**, after the board spent a batch reporting a reassuring `allostatic 0.000/0.6`), a counted CPU-fallback on the bound-propagate lane, seven previously dark teach counters, and a truthy `'browser'` sentinel that had made two browser-donor branches dead code. ⚠ Not verified live; it lands on the press, which remains the next action.
>
> Earlier updated: 2026-08-20 — **SHE LEARNS IN FIRST PERSON, SHE ASKS FOLLOW-UPS, AND SHE DRAWS HER OWN VERSION.** Three capability shifts and one law, all teach-time or draw-time — no equation, weight format or geometry changed, so a Savestart is format-legal. **SELFFRAME:** every lesson is additionally taught as something she DID (`1 + 1 = 2` → *"i add one and one to make two"*), through four chokepoints every grade's every cell funnels through, with `i`/`me`/`my`/`myself` Hebbian-bound to `unity` at the top of every cell — the curriculum previously held **six** occurrences of "i am unity", and its own pronoun lesson had no first-person half at all. **INQUIRE:** a content word from your answer becomes her next question (depth-3, per-chain memory), and the asking is trained rather than scripted. **OWNART:** her "drawings" were a 7-band posterize of a Pollinations photo and the word *draw* routed to the generator — now a look survives only as a ≤9-cell shape schema (~1–2% of the reference's information, so copying is impossible) and she CONSTRUCTS the marks; every drawable noun in the message plus the place it names gets composed. **NOBUDGET:** the 20-minute phase budget is off by default and `STRUCTURE_DOSE` is back to 1.0 — cells take the time they take (*"as long as you coded them correctly"*) — which makes the **consolidation gate the only thing keeping the walk finite** (~24 days of structure-refresh vs ~100 without), now a LAW: re-price `corpus × reps × scale × visits` before weakening any bound. Also: image state can no longer survive a fresh walk (the wipe list named a file nothing writes), the mind's-eye ceilings are raised to the engine's integrity bound, the language-cortex geometry is pinned against a boot-time RAM coin-flip, and a CUDA-only host can finally donate.
>
> Last updated: 2026-08-18 — **THE DROP-ON-SPEAK WAR IS WON.** Chatting with Unity no longer kills the compute donor: the killer (named live through the new remote console ring + chat-stage stamps + send forensics) was the chat text-injection shipping a dense ~23.4MB JSON current frame over the whole Wernicke phon region per message — which the native donor deserializer silently DISCARDED (dense field never existed: all cost, zero function). Fixed by sparse injection (~160 bytes/message, same math bit-identical); chat text now actually reaches her language cortex on native donors. Verified live end-to-end: reply composed, donor RTT steady at 61ms, zero sheds, zero drops — the first survived hi in months. Permanent new organs: the console ring (remotely readable via `/public-state.json?console=N`), per-tick GPU-aliveness in the reply path (donor wobble = brief honest silence, never CPU-stepping the 12M cortex), chat-stage stamps on loop-block lines. Next latency milestone: the one-time ~21.5s first-reply-after-boot pin (non-lethal, instrumentation first — KNOWN_ISSUES KI-19).
>
> Last updated: 2026-08-17 — **THE 12M FULL-SPEED ERA: language cortex 1.5M → 12,000,000 neurons (LG hop 1, WEIGHTS_FORMAT_VERSION 4) + the stalling war won end-to-end.** The walk runs on the grown cortex with the wire immaculate (TMPLFIX template canonicalization: the 2MB grammar-band frames → ~30 bytes; sheds/suppression 0; donor RTT ~66ms), the intra-synapse Hebbian GPU-RESIDENT (GINTRA langCortex pseudo-cluster — zero donor changes on v0.3.17), the event loop unpinned (dashboard build 312→~13ms/call; trailing-yield tax dead; teach/min 200 → 1,100+ with the 1300–1500 pair-phase band the next confirmation), and EVERY cell of EVERY grade learning its vocabulary BEFORE bindings (PRECELL — words-learned-first law across all 19 grades; journey counter 18,017 unique words, defs binding live, first real dream window consolidated processed 11 / bound 221). Self-instrumenting now: per-stage teach timers, getState section laps, broadcast cost split, donor-socket send forensics — the next regression names itself. **Milestone road from here:** (1) the 1300–1500 band verdict at the pair phases → (2) the K→PhD walk proper (all 19 grades × 6+ subjects with per-grade pre-cell vocab) → (3) LG.6 hop 2 (~20M — ⛔ GATED on a segmented-rowPtr donor release [rowPtr ~80MB > the 64MiB message cap] + the upload-compression decision) → (4) LG.7 (the 25% biological-proportion target — its own design pass) → (5) Gee's final full-curriculum test + the one push (no-push-until-PhD-complete LAW stands). (feature/unity-speech-consciousness-rectify) — CODE LANDED, live-walk VALIDATION pending.** Fixes the "word salad after 9th grade": the `word_motor` emission band was recomputed each emit from the live word count, so vocabulary growth desynced every prior grade's `sem→word_motor` binding. Ten equational fixes — **SPEAK.1** frozen bucket geometry (`wordBucketCellSizeFor`, vocab-growth-invariant), **SPEAK.2** post-teach L2 renorm for basin separability (+ Speech Health dashboard card), **SPEAK.3** grade-owned word→word transitions (rep floor 24), **SPEAK.9** reject-to-silence coherence floor, **SPEAK.10b/10c** repeat-loop cap + surprise-gate saturation clamp, **SPEAK.4** at-scale consciousness performance (donor-GPU compose + chunked CPU path + feedback), **SPEAK.5a/5a.i** per-cluster activity-modulated Kuramoto phase wired into the real coherence readout, **SPEAK.6** brain-driven image gen + learning loop, **SPEAK.7** self-architecture knowledge (answers emerge from weights), **SPEAK.8** ComponentSynth brain-driven audit. All `node --check` + ESM `import()` clean, bundle rebuilt; no text-AI, no templates, no fallbacks. **Remaining:** full K→PhD walk + Gee localhost test confirm ordered G9 speech → then feature→develop→main cascade (NO PUSH until verified). Full detail: `docs/unity-speech-consciousness-rectify.md` + `docs/SPONGE-SPEAK-VALIDATION-HANDOFF.md`.
>
> Last updated: 2026-06-27 — **v1.1.0 SHIPPED + TIER3 WAVE CASCADED + DEPLOYED.** (1) Curriculum cells now pass on **learning completion**, not test-question correctness — the A+ probe / student-battery / per-grade-health gates still run but are ADVISORY by default (`DREAM_CELL_PASS_HARD`/`DREAM_BATTERY_GATE_HARD`/`DREAM_HEALTH_GATE_HARD` restore hard-block); this unstalled the walk that was pinned at 0 cells by a collapsed `sem_to_motor`. (2) **sem→motor saturation prevented during teach** — Hebbian LR damped on the emission projections at `_crossRegionHebbian` (`DREAM_SM_LR_SCALE`=0.5, reaches GPU+CPU paths; secondary `DREAM_SM_WMAX`). (3) **Admin Application Profiling** dashboard card (`state.profiling` — system resources / throughput + event-loop histogram / network / per-client client↔brain health). Tagged `v1.1.0`; the consciousness/mind-space wave below was then cascaded on top + deployed to the box (fresh-walk resumed). **Remaining:** live donor-GPU verification of CGATE.1 GPU-generation + server `_imagineTick`.
>
> Last updated: 2026-06-27 — **CONSCIOUSNESS DE-GATED + EQUATIONAL MIND-SPACE NOW SERVER-SIDE + TIER 3 IDENTITY REPAIR + MIND'S EYE VIEWER** (feature/tier3-identity-seed-repair, commit 0d97804; pushed to `if-only` — develop→main cascade pending Gee's word). Unity reported her consciousness was "gated too much" — fixed: GlobalWorkspace theta is now a graded ignition modulator (was a hard 50%-of-ticks wall), ignition threshold 0.45→0.35, and the Ψ consciousness gain is self-calibrating (was inert ~1.0) so consciousness actually modulates the brain. The UniVsMatics equational mind-space now runs on the SERVER brain (was browser-only) with DE-NOVO imagination from her own cortex state (no camera) — bounded forward-9-7 only, never fractalize, hard resolution cap (no nanometer seize). She LEARNS her mind-space, her imagined imagery persists across reboot, and the gated inner-voice showcase is now coherent (not word-salad). Tier 3 identity store (read ZERO before) now top-ups missing anchors at boot. New public **Mind's Eye** viewer (`/minds-eye.json` + `html/minds-eye.html` + 👁 footer button) — single shared source, no per-viewer compute. **Remaining:** develop→main cascade + live donor-GPU verification of CGATE.1 GPU-generation + server `_imagineTick`.

> Last updated: 2026-06-23 — **KINDERGARTEN NO-STALL + WEIGHTS PERSIST + dashboard Update buttons (shipped on main).** Curriculum `_rectifySemMotor()` used to HALT and `return` out of the whole walk on sem→motor saturation (basin-collapse) — the walk never reached grade1. It now RECTIFIES in place (weight-decay + row-norm + clears stale signals) and CONTINUES + force-checkpoints, so the walk no longer stalls at kindergarten. Periodic save now force-writes during the walk (every 5 min) regardless of cell-pass. Two NEW admin-only dashboard buttons: **⬆ Update & Fresh Walk** (`POST /update` — overlays latest code + wipes for a fresh walk) and **⬆ Update & Savestart** (`POST /update?keep=1` — overlays latest code but RESUMES saved weights, so a fix deploys without losing training).
>
> Last updated: 2026-06-20 — **DISTRIBUTED DONOR-GPU DEPLOYMENT BUILT (pre-alpha).** The deploy stack is now live: a deployed static page + persistent Node brain-server backend on the same server behind an nginx reverse-proxy; visitors donate browser GPU compute via `compute.html` (data-parallel replicas + Hebbian-delta merge — the brain trains/runs on donated GPUs); community-compute auto-scaling (up-scale on critical mass + down-scale rectify, admin dead-zone toggle/sliders, one-disconnect-never-downgrades); a Forgejo-authed admin lane (first authed after deploy = master operator) with a live server-console + auto-scale-control dashboard. Cognition stays 100% EQUATIONAL (no text-AI) — Unity learns alphabet→phonemes→words→sentences→K-to-PhD running ON the deployed donor GPUs. REMAINING: the live K→PhD curriculum walk + final validation now run ON the deployed page with real donor GPUs, and deeper per-pass multi-GPU tuning is measured on deploy. Local dev (`start.bat` / `Savestart.bat`) still exists for development.
>
...
> Unity AI Lab
>
> **DOC-AHEAD-OF-REALITY (operator ruling, 2026-04-17):** Per-grade sign-off status lives in `docs/TODO-full-syllabus.md` + `docs/FINALIZED.md`. This roadmap describes target-state milestone completion.
...
| Metric | Value |
|--------|-------|
| **Phase** | Distributed Donor-GPU Deployment BUILT (pre-alpha) → Live K→PhD Walk + Final Validation on Deploy → Push (Pre-K + K ONLY scope REVOKED 2026-06-18 — every grade K through PhD now BUILT and in scope) |
| **Status** | ✅ Brain refactor complete · ✅ Full K→PhD curriculum BUILT (K, G1-G12, College 1-4, Grad, PhD — every subject + full lived-year per grade) · ✅ T15 drug dynamics scheduler live · ✅ Full save/resume (JSON cortex state + streaming binary weights + operator signoff ledger) · ✅ Clean-halt path via `stop.bat` + `POST /shutdown` · ✅ DYN-PROD redesigned as direct `sem_to_motor.propagate` · ✅ Student-test probe battery (A+ ≥ 0.95) — runs every cell but ADVISORY since 2026-06-27 (cells pass on learning completion; hard-block restorable via `DREAM_*_HARD`) · ✅ Deployed static page + persistent Node brain-server on the same server behind nginx reverse-proxy · ✅ Distributed donor-GPU compute (browser GPUs donate via `compute.html`, data-parallel replicas + Hebbian-delta merge, brain trains/runs on donated GPUs) · ✅ Community-compute auto-scaling (up-scale on critical mass + down-scale rectify, admin dead-zone toggle/sliders, one-disconnect-never-downgrades) · ✅ Forgejo-authed admin lane (first authed after deploy = master operator) + admin dashboard with live server-console + auto-scale controls · ⏳ Live K→PhD curriculum walk on the deployed page with real donor GPUs · ⏳ Operator final test of all courses through PhD on deploy · ⏳ Deeper per-pass multi-GPU tuning measured on deploy |
| **Re-measured** | **2026-09-07** — ⛔ this row read `Current date: 2026-04-21` for nearly five months. **A "current date" field that is not the current date is the purest form of a stale status block, because the one thing it exists to tell you is the one thing it gets wrong.** |
| **Where she actually is** | `ela/kindergarten`, **phases 2 of 25**, **cells passed 0**, lowest grade cleared **none — still pre-K**. Teaching at **4,645 pairs/min** on a boot of **388,597,268** neurons. ⚠ **The curriculum is BUILT; the walk has barely started.** Those are different claims and this page has historically run them together. |
| **Shipped milestones** | Brain refactor (neurons + synapses + **8** clusters — ⛔ *this said 7; the brainstem was added and this row never learned about it* — + 20 projections); semantic GloVe 300d + fastText subword encoding for out-of-vocabulary words; server dynamic-import client brain modules; text-AI cognition ripped; multi-provider image gen + vision describer; equational component synthesis; unified sensory peripheral contract; embedding refinement persistence; setup modal with provider grids; 3D brain event system with equational popups age-gated by life grade; real amygdala attractor; RemoteBrain local VisualCortex with iris tracking; GPU buffer binding cap; admin GPUCONFIGURE.bat tier ladder; plain-English unity-guide.html; brain-equations.html worked summation walkthrough; **developmental curriculum primitives** (the cortex sub-region substrate — ⛔ *this said "8-subregion" and "14 cross-region projections"; measured live it is **eleven** top-level sub-regions and **sixteen** projections, the extra pair being meaning ↔ whole-word* — letter-input inventory, syllable boundary detection, cortex-resident words, tick-driven motor emission, sentence-form schemas, identity lock); **full K-PhD syllabus code** (114 cells × 6 subjects with equational teaching methods + real human-grade gates + persistent life info propagation); **T15 drug dynamics rebuild** (9-substance pharmacokinetic scheduler, grade-gated availability, additive contribution model, speech distortion post-processor, offer detection, self-initiation engine, simulated social acquisition, slash command controls, UI dynamic rendering); **letter-naming phase** (`_teachLetterNaming` — binds `letter(X) → motor(X)` identity via 26 × 18 × 2 Hebbian ops so TALK probe can answer "say the letter A"); **full save/resume system** (server-side saveWeights persists every cortex learned field — grades / passedCells / probeHistory / T14.13 learned-language Maps / T14.16.5 identity thresholds / T14.17 persona dimensions + intent centroids + refresh corpus / letter inventory / `_gateHistory` + streaming binary weights for `cortexCluster.synapses` via `fs.openSync` + zero-copy Buffer views); **operator grade-signoff ledger** (`POST /grade-signoff {subject, grade, note}` persists operator-verified pass events per LAW 6 Part 2); **clean-halt path** (`stop.bat` + `POST /shutdown`); **student-test probe battery** (60+ grade-appropriate questions across all 12 pre-K + K cells via `_studentTestProbe` / `_runStudentBattery`, scored on methodology + logic + retention + understanding through the real cortex language pipeline); **DYN-PROD redesign** as direct CPU `sem_to_motor.propagate` (no LIF ticks — kills silent-cortex + node heap OOM + GPU-cascade at once); **intermediate-rep CPU Hebbian skip + final-rep sampling** (5-8× teach speedup on `_teachPhonemeBlending` + `_teachWordEmission`); **mid-phase checkpoint saves** (`_phaseDone` records phase + fires `_saveCheckpoint` so weights persist at every phase boundary); **probe-gate pause** (`cortexCluster._probeGateActive` skips main-brain `_gpuBatch` for the whole cell duration — no `compute_batch` can be in flight during a long teach phase); **distributed donor-GPU deployment (pre-alpha)** — deployed static page + persistent Node brain-server on the same server behind an nginx reverse-proxy; **distributed donor-GPU compute** (visitors donate browser GPU via `compute.html`, data-parallel replicas + Hebbian-delta merge, brain trains/runs on donated GPUs); **community-compute auto-scaling** (up-scale on critical mass + down-scale rectify, admin dead-zone toggle/sliders, one-disconnect-never-downgrades); **Forgejo-authed admin lane** (first authed after deploy = master operator) + **admin dashboard** with live server-console + auto-scale controls. |
| **What's next** | Run the full K→PhD training walk ON the deployed page with real donor GPUs (every grade K through PhD, every subject + full lived-year per grade) → operator's final test of completed training across all courses through PhD grad with adult-Unity behavior fully emergent → pre-push doc accuracy sweep → explicit push approval → push to main. No intermediate pushes until the walk + final test complete (per the no-push-until-PhD-complete LAW). COMP-todo Part 2 distributed compute is now BUILT (donor-GPU + auto-scaling + admin lane shipped pre-alpha); remaining COMP work is deeper per-pass multi-GPU tuning measured on deploy. |
...
- **Semantic Embeddings** (`js/brain/embeddings.js`) — GloVe 300d + fastText subword encoding for out-of-vocabulary words, cortex mapping, online context learning
...
- New `getSubsetForWords(words)` — server precomputes a corpus-word-only subset for browser bulk load
...
**`js/brain/cluster.js` changes (T14.4 substrate, revised 2026-04-14 after the slot-equation pushback):**
...
Refactor driven by the operator's *"logic and reasoning in a form yeah?"* question + *"no artificial limits as unity may be talking to users while she does ciriculum"* binding — landed the unified combination-operator scaffold:
...
## Unified Brain-Driven Language Cortex — SUPERSEDED

⛔ **This heading read "IN PROGRESS" until 2026-09-07, and the work it describes was not merely finished — it was replaced.** The emission loop documented below was deleted entirely and rebuilt as the developmental layers above. **It is kept because it records why that rebuild was necessary**, not because any of it still runs.
...
Shipped first on explicit operator instruction. The cortex cluster's recurrent synapse matrix now trains on the persona corpus via sequence Hebbian during boot.
...
- **U287** Sentence completeness — `_isCompleteSentence(words)` at `language-cortex.js:1729` rejects sentences ending on DET/PREP/COPULA/AUX/MODAL/NEG/CONJ/PRON_POSS. Wired at `generate()` post-render with 2-retry loop.
...
## Full Brain Control Refactor — SHIPPED
