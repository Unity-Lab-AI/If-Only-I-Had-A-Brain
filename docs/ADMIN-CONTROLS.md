...
| `DREAM_FIRING_TARGET_PCT` | **7.5** (`0` disables) | FIREKNOB — the firing-rate controller's target: what percent of stepped neurons should be firing at any moment (operator's band: *"should be like 5-10% of the brain at any moment"*). Every answered step batch measures the true fired fraction, EMA-smooths it, and nudges a bounded drive scale (×[0.25, 10], ±15%/sample) on every cluster's `tonicDrive` until the measurement meets the target — the psiGain/SUBSTEPS.2 self-calibration idiom, never a hand-tuned constant. Live at `state.firing` (pct/ema/target/driveScale/samples); a scale pinned at a bound means the target is beyond the drive knob's reach and says so. `0` pins the scale at 1.0 — the exact pre-knob physics |
