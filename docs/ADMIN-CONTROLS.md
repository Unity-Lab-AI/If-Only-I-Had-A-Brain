...
| `DREAM_CHAT_QPROBE_TIMEOUT_MS` | ✅ `20000` | lever | Hard budget on the chat question lane — a question-like message runs the gate battery's own probe (same teach-geometry injection + trained-pathway reads the exam uses) before the compose lane; past the budget the probe is aborted and compose runs exactly as before |
