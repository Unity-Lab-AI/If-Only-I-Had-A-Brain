...
| `DREAM_RANGE_MAX_RUNS` | **`65536`** | `SHADOWCOST.5`/`.7` — how many `[start,len]` runs a teach pattern may compress to before the GPU refuses it and the CPU runs the full pass. ⛔ **This is OURS, not the donor's** — `Work::HebbianRanges` caps total expanded indices at 2M and any single range, with no cap on the NUMBER of ranges. At 512 it was sending **70.4% of the heaviest op to the CPU**; at 65,536 the measured worst case (51,330 runs) dispatches for **1.01 MB / 27 ms of wire against an 806 ms CPU pass**. Re-derive from `intraOja.rangesRunsMax` rather than doubling on instinct |
