...
> ⭐⭐ **2026-09-06 — THE PANEL IS WRITABLE FOR THE FIRST TIME, and it was one gate away the whole time.** `writable` is `unproven === 0` — **all or nothing** — so **twelve** knobs with no proven effect class held the other **205** read-only, on a box with no shell, in a panel built for exactly that reason. All fourteen (the twelve plus two that were invisible) were read at their sites and curated: **total 217 → 219 · unproven 12 → 0 · described 211 → 219 · `writable` false → TRUE**, and the `UNSORTED — no category read yet` group went to **0 members and disappeared**.
>
> ⛔ **The effect class was never inferred from indentation** — this project already wrote a brace-depth classifier and discarded it for misreporting two knobs as `live`. The only test applied: **does writing this value NOW change behaviour LATER?** That is why `DREAM_SEM_TOPOGRAPHIC`, `DREAM_TOPO_RADIUS_FRAC` and `DREAM_LAMINATION_VETO` are **`boot`** despite being read inside functions — all three decide how a matrix is **built**, so a later write reads back correctly and does nothing.
>
> ⚠ **And two knobs were invisible because they were read through an alias.** Discovery matches `process.env.X` or a helper call; `const _env = process.env; … _env.DREAM_LATERAL_HINT` is a third pattern and neither regex sees it. The panel reported 217 and held 219. Fixed at the call site — `process.env.X` is the convention, and a scanner chasing arbitrary aliases would be guessing.
...
| `UAL_FIELDS_HYDRATE` | **`0` — OFF** | ⭐ **THE CREDIT-FREE FIELD HYDRATION, AND IT IS OFF ON PURPOSE.** `1` copies Forgejo's own LFS objects into place **by OID off local disk** — no git-lfs, no network, no deploy key — which is the only way fields ever arrive on this box. ⛔⛔ **IT WAS DEFAULT-ON FOR ONE PRESS AND THAT PRESS COINCIDED WITH A 20-MINUTE OUTAGE:** the loop copies up to ~100k files / ~114 GB, and while the deploy shares her cgroup **every byte is page cache charged to her `MemoryHigh`** — the hazard is page cache, not CPU or disk, and `ps rss` looks innocent throughout. ⚠ **Not proven** (the same listening-but-not-answering signature is also a normal long resume) but it is the leading explanation: the boot four minutes earlier answered state in 2.5 minutes with this loop as the only difference. ⭐ **The asymmetry decides the default: a missing field costs one live transform, an unreachable brain costs everything.** ⛔ **Turn it on only after the deploy's own first log line reads `cgroup: CONTAINED`** — and it does **not** flip itself on that reading, because a false positive puts the 114 GB back in her budget. The `OFF` log line reports whether the precondition is met, so the decision comes off the log rather than out of memory |
| `UAL_FIELDS_HYDRATE_MAX_BYTES` | **2 GiB uncontained · `0` contained** | ⭐ **THE PRIMARY BOUND ON THE HYDRATION, AND BYTES ARE THE RIGHT UNIT BECAUSE PAGE CACHE IS THE HAZARD.** ⛔ **This replaced a 480s wall clock that was the wrong INSTRUMENT, not merely the wrong number** — seconds only become page-cache volume by way of disk speed, so `480s` is ~480 GB of cache at 1 GB/s and ~4.8 GB at 10 MB/s: **the same bound, two orders of magnitude apart in the thing it exists to limit.** **Derived, not picked:** the recorded incident is 12.4 GB of page cache pinning the cgroup with node at only 8.7 GB RSS; against `MemoryHigh=22G` and a ~8.8 GB node the whole headroom is ~13 GB, so 2 GiB leaves it standing. At ~1.13 MB a field that is ~1,800 fields a press — a trickle, deliberately. Checked **per file**, so a fat copy cannot overshoot by a batch; **the remainder is counted and logged with its number**, because a bound that truncates silently reads as *"we hydrated everything there was"*. `0` disables it |
| `UAL_FIELDS_HYDRATE_MAX_SEC` | **90 uncontained · 480 contained** | **The second net, for the slow-not-large case.** Re-derived **for a copy**: 2 GiB at the 25–100 MB/s a `nice -n 19 ionice -c3` copy actually gets is 20–80s, so 90s is the smallest clock that cannot cut a healthy run short — and it is ~13× shorter than the outage that made the hydration default-off, **which is the property a guard needs and the one 480s did not have.** ⚠ **A constant carries the conditions it was measured under:** 480s came from the LFS pull, where eight minutes is short *for a download*. Whichever of this and the byte budget bites first stops the loop, and **the log names which one** — two bounds with one message is how the next person raises the wrong knob. `0` disables it. ⭐ Both bounds cost nothing permanent: already-full-size destinations are skipped, so the hydration is **incremental by construction** and the next press resumes where this one stopped |
| `UAL_DEPLOY_OWN_CGROUP` / `UAL_DEPLOY_MEM_MAX` | **on** / **`2G`** | **Whether the deploy gets its own cgroup, and how much memory it may have.** The `/update` handler launches `self-update.sh` in a `systemd-run --user --scope` so a heavy clone/rsync/copy spends the **deploy's** budget instead of hers — `detached: true` starts a new *session*, not a new *cgroup*, and systemd membership is inherited by every descendant. ⛔⛔ **IT WAS INERT ON THIS BOX FOR THREE DAYS WHILE THE LOG CLAIMED IT WORKED:** `--user` needs a user-manager bus and a **system** unit supplies neither `$XDG_RUNTIME_DIR` nor `$DBUS_SESSION_BUS_ADDRESS`, so it exited 1 having started nothing and the plain-spawn fallback ran the deploy inside her cgroup on every press. ⛔ **ONE COMMAND FIXES THE REST, ONCE, AS ROOT: `sudo loginctl enable-linger unity`** — that creates the `/run/user/<uid>` a service account with no login session otherwise lacks, and it persists across reboots. ⭐ **Do not read containment off this row or off the handler's summary line** — read the deploy's own first log line, `cgroup: CONTAINED` / `cgroup: ⚠ UNCONTAINED`, which it takes from `/proc/self/cgroup` **and** its own `memory.max`. **Both halves are load-bearing:** being outside her unit is not enough, since a hand-run copy from SSH is also outside it and has no ceiling at all |
...
| `DREAM_CONSOLIDATION_WATCHDOG_MS` | **300000** (5 min) | ⛔⛔ **THE BOUND ON HOW LONG THE WALK WAITS FOR A FORCED PASS — added 2026-09-06 after a LIVE 110-MINUTE WEDGE.** The dream window awaited the pass with no timer, stated in its own comment as a design choice (*"No wall-clock timer"*). Measured on the box: the walk sat between phases for **110 minutes**, `hebbian.calls` frozen, while the heartbeat printed, the donor computed at 259 ms/batch and the event loop stayed clean — **nothing had crashed; execution was parked on one line.** ⚠ **And the damage exceeded the pause:** the window's `finally` restores `_curriculumInProgress` and `_operatorSleepRequested`, and a `finally` does not run until its `try` settles — so one unbounded await left the brain flagged asleep and **silently disabled teaching, the deferred-lane drains, drawing and the mind's eye at once.** ⛔ **The engine already promised 120 s and the await outlived it by ~55×** — *a bound inside the thing being bounded cannot catch the case where that thing stops making progress*, which is why this one is outside. ⚠ **It does NOT cancel the pass**; the pass keeps running and the next window declines via the engine's `_inFlight` guard. On a trip the window skips its remaining stages — because the abandoned pass is still running and the trickle teaches, and two teachers on one substrate is the hazard. Read `curriculum.consolidationWatchdog.trips`; **`0` is healthy and is published beside `watchdogMs` so "never needed it" and "never had it" are distinguishable.** Derivation in `docs/THRESHOLD-DERIVATION.md` |
...
| `DREAM_REP_AUTOPRICE_GAP_MS` | **60000** (60s) | ⛔⛔ **PER TEACH LANE since 2026-09-06, and it was per BRAIN before — which is why this instrument had never answered the question it was built for.** One global clock plus one sticky verdict slot meant that in any window exactly ONE lane could be measured, and whichever called first took it. **Live off the box after 76 minutes of teaching: `measurements: 1`** — from a **closed-class** lane of 36 words with `maxSharers 36` (every pattern on one dim) reporting a load of **74.8**. ⚠ **That is the SKEW case the counter exists to catch, arriving dressed as the corpus**, and the prose lanes whose cost the setting governs had never been measured once. Harnessed on the shipped block: a skewed lane reads **312** where a spread lane reads **5.45** in the same brain — a **57×** spread no single slot can represent. Read the per-lane book at `state.curriculum.repPricingByLabel`. How often the collision-load measurement re-runs. ⛔ **It previously ran on EVERY qualifying call** — 25,603 of them in one live boot — sampling up to 512 patterns each, where a sample costs a fresh `Float64Array(300)` plus a GloVe lookup plus five hash stripes plus a top-K pass. ⭐ **And the result went nowhere:** it was written to a property with exactly one reference in the whole tree, the write itself, so the brain paid a per-call sampling tax to compute a number it then discarded. The measured quantity is a property of the corpus and the encoding, and neither moves between two calls a millisecond apart; the fastest thing that legitimately changes it is a grade transition, and cells run tens of minutes. Derivation in `docs/THRESHOLD-DERIVATION.md` |
...
| `DREAM_READBACK_IDLE_MS` | ✅ `120000` (2 min) | escape hatch | How long the donor may go **silent** during a GPU values readback before the pull is abandoned. ⚠ **This is an IDLE window, not a transfer budget** — the deadline re-arms on every chunk, so a slow-but-moving transfer is never killed. That distinction is the whole fix: the previous fixed 600 s deadline aborted three live pulls *while chunks were still arriving* (80–132 chunks, 0.67–1.1 GB in), so the largest matrix could never finish and the checkpoint's record of GPU-trained weights decayed. **Derived, not picked:** at the measured ~39 MB/s with sub-megabyte chunks, chunks arrive milliseconds apart, so 120 s is ~4 orders of magnitude above the healthy gap — past any GC pause or teach-loop block, while still failing fast on real silence. Raise it only if a donor legitimately pauses longer than two minutes mid-transfer |
| `DREAM_WRITE_PRACTICE_GAP_MS` | ✅ `1800000` (30 min) | lever | Minimum gap before she practises writing the same text again. Writing practice is the trained skill on top of the letterforms — she renders her own handwriting, **perceives it**, and scores resemblance against the printed form, keeping only nudges that measurably improve it. The gap stops one word monopolising the shared walk lane. `0` allows immediate repeats |
| `DREAM_WRITE_PRACTICE_ITERS` | ✅ `5` | lever | Nudges attempted per writing-practice session. Each is one render + perceive + score cycle at 256 px on the walk lane, so this is the direct cost knob. ⚠ Raising it does not make her improve faster in a straight line — a nudge is only kept when it beats the current best, so the ceiling is what the parameters can actually express, not how many tries she gets |
| `DREAM_WEIGHT_PRUNE` | **ON** (`0` disables) | escape hatch | ⭐ **`WEIGHTFIT` (2026-09-24) — fit the saved weights under the cgroup ceiling at apply time.** The applied weights are RESIDENT; a file that grows with learning eventually will not fit under `memory.high` at the neuron count that produced it, and the boot pins at the ceiling (measured four times that day, 21 GB of 22.5). With this ON the restore reads its own cgroup with the brain arrays already resident and, ONLY if the sections will not fit, drops the smallest-\|weight\| synapses uniformly across sections until they do — neuron count untouched, so nothing reads as incompatible and nothing wipes. Fits → nothing pruned. `0` loads the weights whole and says so in a WARN. Boot line: `WEIGHTFIT — fits` or `WEIGHTFIT — the saved weights do NOT fit … Keeping the largest N%`. |
| `DREAM_WEIGHT_FIT_PCT` | ✅ `85` (50–100) | tunable | The share of `memory.high` the resident brain may occupy after the weights apply. ⚠ **A chosen margin, not a measured one** — measured points are 58% (healthy boot 2026-09-09) and 91–102% (pinned, 2026-09-08 and 2026-09-24); the knob exists so this moves when a point in between is measured. Below the value the kernel has page-cache room for the corpora reader and saves; at 91%+ she has pinned every time. Derivation in `docs/THRESHOLD-DERIVATION.md`. |
| `DREAM_CGROUP_OVERHEAD_MB` | ✅ `2867` | escape hatch | How much of the **cgroup** memory limit is reserved for everything in this process that is NOT weights — V8 heap, buffers, corpus. The boot budgets against the smallest of the host reserve and `(cgroup limit − this)`, because the kernel enforces the cgroup while the sizing used to reason only about the host: a budget of 18,519 MB produced a **21.3 GB** process and sat 906 MB over `MemoryHigh=20G`, throttled by a ceiling it could not see. **The default is measured, not picked** — it is the gap between that live budget and the RSS it produced. ⚠ Raise it if RSS still overshoots the cgroup limit; lower it only with a fresh measurement of RSS-minus-budget in hand. ⛔ Off-heap `ArrayBuffer`s count against the cgroup but **not** against `--max-old-space-size`, which is how a "16 GB heap cap" produced a 21.3 GB process — so this, not the V8 flag, is the bound that matters. Ignored where no cgroup exists (local Windows/macOS dev). ⛔⛔ **AND THE ANECDOTE IN THIS ROW IS NOT HISTORY — IT IS LIVE, MEASURED 2026-09-08.** She still budgets **18,519 MB** against a `MemoryHigh` of **20,480 MB**, i.e. `20,480 − 2,867 = 17,613 MB` safe versus 18,519 taken — **the same 906 MB overshoot, still there, still by construction.** Her boot log names why: `CGROUP-AWARE SIZING — kernel limit 22528MB … vs host reserve 18519MB → budget basis 18519MB, bound by the host reserve` — **the sizing budgets against a ceiling that is not the one that throttles her.** Live consequence at `01:25Z`: `memory.current` 20.58 GiB, **+628 futile high-throttle events per second**, proc state `Dsl`, `/health` TIMEOUT, `LoopWatchdog … STILL STALLED`. ⛔ **She cannot recover from it** — the cgroup has `file 0` and the host's 1 GB of swap is **99.99% full with none of it hers** (`memory.swap.current = 0`), so reclaim has nothing to reclaim. ⚠ **Raising the limit instead is NOT the fix and has been tried: it caused a host-wide OOM kill** (`global_oom`, `oom_kill = 0` in her own cgroup) because a cgroup ceiling above the host's real free memory is not a ceiling. ⛔ **And lowering the budget changes the neuron count, which wipes the trained weights** — see `deploy/dropins/10-pin-brain-size.conf`. Tracked as **`KI-42`**; options and their costs are tabulated in `docs/RESUME.md §2026-09-08 (15th)` |
