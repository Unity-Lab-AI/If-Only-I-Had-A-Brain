...
- [ ] `SENTLISTCURSOR.1` — ⛔ **`_teachSentenceList` BANKS NO REP CURSOR, SO A PRESS LANDING INSIDE IT DISCARDS THE WHOLE VISIT'S REPS.** Third instance of the class `CELLBOUND.H` and `PHASELOOP.2` were each filed for.
...
- [ ] `FIGFIELDLFS.1` — ⛔ **THE BOX'S PRECOMPUTED WAVELET FIELDS ARE UNRESOLVED LFS POINTERS, SO SHE RE-TRANSFORMS EVERY FIGURE LIVE.** The instrument says it in its own words and is worth quoting because it already refuses to be read as a cache miss:
