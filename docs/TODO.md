...
- [ ] `CURVEDEPTH.11` — ⚠ **THE TEXTBOOK HALF OF THE COLLEGE→PhD DECISION IS THINNER THAN IT LOOKED, AND THE NUMBERS ARE NOW MEASURED.** Filed as the successor to `CURVEDEPTH.6` so a half-built decision is not read as a finished one. **Probed before building: 250 of Open Textbook Library's 2,005 books, through its JSON API.** ⛔ **Only 27% (67/250) carry a commercial-safe licence** — CC-BY / CC-BY-SA / GFDL; the rest are **CC-BY-NC-\***, which this project's licence posture excludes — **and only 23 of 250 are both licence-safe AND in a subject this corpus needs**, extrapolating to **~184 usable books catalogue-wide**. ⭐ **Worse for the purpose: the largest host in that usable remainder is `openstax.org` (25 of the format links), i.e. books this corpus already ingests**, and most "Online" links point at **LibreTexts**, which the gap ledger already excludes on the same licence ground. **So OTL's true addition at college2-4 is a fraction of ~184 books minus what OpenStax already gave us.** ⚠ **This is Gee's call to revisit, not mine to take** — his option B, **DOAJ** (open-access journals, every discipline, one fetcher), is the obvious cover if the OTL slice proves too thin, and the college2-4 band is the one place where journal articles are a poor stand-in for a course. **Bring him the numbers, not the question cold.**
