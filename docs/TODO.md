...
**⚠ Deploy reality:** this is a BACKEND fix (the walk runs server-side in `/opt/unity-brain`'s `curriculum.js`). Per the #112 constraint, backend redeploy/restart/env changes are BLOCKED until the box admin returns — so this stays staged until the box is reachable. The frontend `git main` rsync does NOT update the Node backend.

- [~] **#112.9a — §6.1 per-question budget + §6.2 battery deadline (DONE on `feature/student-battery-timeout`, staged for box redeploy).** `_runStudentBattery`: per-question `Promise.race` wall-clock cap (`_probeWithBudget`, `DREAM_BATTERY_QUESTION_TIMEOUT_MS` def 45 s) with an `AbortSignal` threaded into `_studentTestProbe` (checkpoints before each `generateSentenceAwait` + the methodology pass, so a timed-out probe stops competing for the GPU); battery-level wall-clock deadline that actually consumes `_batteryStart` (`DREAM_BATTERY_DEADLINE_MS` def 8 min) — past it, remaining questions degrade to score 0 and the cell returns. Timed-out / deadline-exceeded → score 0, never an unbounded await. Defaults baked in (no env needed). `node --check` clean. Server loads the SOURCE module (`brain-server.js:1129 import('../js/brain/curriculum.js')`).
- [~] **#112.9b — Item 1: battery-gate ADVISORY mode (DONE, default OFF).** `DREAM_BATTERY_GATE_ADVISORY=1` keeps blockers computed/logged/recorded in `_lastGateResult` but does NOT downgrade `result.pass` — the substrate gate alone advances, so a weak battery no longer wedges the cell ("if we get a cavewoman so be it"). Default OFF = unchanged hard-gate. **DECISION PENDING (Sponge): ship the live deploy with this ON or OFF?** When ON, relaxes the grade-completion-gate LAW — Gee's call. Needs an env set on the unit = blocked on box admin.
