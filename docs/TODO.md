...
- [ ] **SD.2 — Auto-scale toggle default-ON + persist (Gee 2026-06-30).** Verbatim: *"we need Auto-scale enabled to be default 'on'(check marked enabled) in the community Compute and auto scale so refreshes of the page do NOT uncheck it( currently if refresh the page it uncheckes the auto scale(and auto scale does not persist between admin browsers( ie, if i turn it on and sponge goes to the admin dashboard it should appear as on( currenty its wrong iof sponge goes to admin dashboard or i do, it shows as not checked even if one of us already toggled it on.. so this needs addressed."* Server-side default `enabled:true` in `_getAutoScaleSettings`; dashboard seeds the toggle from `/autoscale` GET on load (not a local default) so refresh shows persisted truth; `/autoscale` POST persists to `server/autoscale-settings.json` + `autoScaleChanged` WS so both admin browsers reflect the same live state. Logged in `docs/SPONGE-DIRTY-AND-0GNS-DIAGNOSTIC-2026-06-30.md` Issue 5 (Sponge-side, server/dashboard wiring).
