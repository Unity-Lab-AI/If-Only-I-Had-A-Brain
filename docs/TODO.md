...
- [ ] `SPONGEHAND.B` — ⏳ **THE RUST REWRITE, B0–B7 — AWAITING GEE'S SCOPE CALL.** Sponge (verbatim): *"**B0. `unity-protocol`** — risk: none … Zero behaviour change. Kills the duplicated protocol. **Start here.**"* and *"**B7. Extract curriculum/vocabulary data** — do this early, it shrinks everything … Worth doing even if the rewrite stalls."*
  - ⚠ **This is a multi-week program against ~42,700 lines of coordinator JS**, and Sponge's own sequencing puts it after section A. **Not started without an explicit go.**
