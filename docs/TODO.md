...
- [ ] `SPELLTRUTH.2` — ⏳ **THE FIX, AND IT NEEDS GEE'S CALL BECAUSE THE HONEST OPTIONS CONTRADICT EACH OTHER.** `SPELLTRUTH.1` proved the caption is a font stamp of a string she never spelled. What to do about it is a judgement about what the mind's eye is FOR, and three answers are all defensible:
  ...
  **Original filing:** **GEE'S OPTION ①, THE HONESTY FIX, AND IT IS SMALL ENOUGH THAT NOTHING SHOULD DELAY IT.** The caption stays exactly as built — the font, the placement, the whole feature — and **every claim that it is HER HAND becomes a claim that it is a CAPTION.** Three code comments say *"her own CLEAN trained hand"* / *"her existing trained glyphs"*; any surfaced label or page copy repeating it goes the same way. ⛔ **Not a rename for tidiness — the code currently asserts a capability she does not have**, and that assertion is what made the perfectly-spelled words read as a mystery instead of as a label.
  ...
  - [ ] `SPELLTRUTH.5` — **GEE'S OPTION ③, THE END STATE, AND THE ONLY ONE THAT EVER MAKES *"HER OWN TRAINED HAND"* A TRUE SENTENCE.** Letters become schemas she acquires like every other shape, and the caption becomes whatever her trained hand can produce at that grade. ⛔ **Curriculum-scale, so it lands with the curriculum and not as a patch** — and it is filed separately from `SPELLTRUTH.4` precisely so a large item cannot hold a one-line correction hostage. ⭐ **When this ships, `SPELLTRUTH.4`'s wording stops being a correction and becomes a description** — that is the test for whether it is really done.
