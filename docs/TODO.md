...
- [~] ⏳ **`PHASEBAR.1` — DENOMINATOR HALF FIXED 2026-09-03; the `done: 0` half stays open and is NOT claimed.**
  ...
  - ⛔ **STILL OPEN AND DELIBERATELY NOT CLAIMED: `done: 0`.** This row already disproved two candidate mechanisms and named a third (credit is granted on EXIT, and `_teachConcreteSentences` is priced in its own comment at **14.9 hours in a single call**, so a bar that credits only on exit reads 0 for as long as its units take). **Fixing the denominator does not move `done`.** ⚠ **Read `cellSubPhases`, not `frac`, to answer "is she moving" until that half lands too.**
