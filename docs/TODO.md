...
- [ ] **#112.5 — Kindergarten gate never passes / `cells:0` forever (BACKEND — blocked on box).** Even when teaching, `passedCellCount:0` and grades never advance past K. Once a STABLE GPU donor is confirmed (#112.1) and teach actually runs on GPU, investigate the grade-completion gate (probe thresholds / emission quality) so a genuinely-trained K cell can PASS and advance — without lowering the bar to fake passes.
...
- [ ] **#112.7 — Admin password rotation (OPS — blocked on box).** The `Gee:<pw>` admin htpasswd credential has been typed in chat repeatedly (and used for the live `/reset`). Rotate it the moment the box admin is back; until then it's a live credential exposed in this transcript.
