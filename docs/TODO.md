...
- [x] `STRANDED.1` — ⛔⛔ **TWO TEACH LANES COULD NEVER REACH PAST THE HEAD OF THEIR OWN MATERIAL, AND ONE OF THEM WAS ANCHORING NOTHING AT ALL.**
  - Gee (verbatim): *"and remember we need to show and train unity on all book's illistrations concurrentlky with the text describing it properly linking them in the traing when unity sees it in the minds eye"*
  - Gee (verbatim): *"okay but u need to check that for all ciriculum reading materals and traing materailas for all phases grades and cells"*
  - ⭐ **THE LINKING HE ASKED ABOUT WAS ALREADY RIGHT, AND IT WAS VERIFIED RATHER THAN ASSERTED.** Same cell pass, prose first then pictures — `_trainAcademicStories()` then `_perceiveCellFigures()`, the second commented *"so a figure is perceived while the words it illustrates are the freshest thing in the cell"*. A figure carries its own label AND `context`, the corpus prose it sits inside, cleaned by the same cleaner that made that cell's sentences; the percept is banked under the cell's own theme and `_queuePhraseTeach` **teaches** those words rather than merely storing them. Measured on the newest cell: **175 figures, 167 with a real label, 159 with real prose, 0 with neither.**
  - ⛔ **BUT THE PICTURES PAST ROUGHLY THE 24TH IN EVERY CELL WERE UNREACHABLE FOR THE WHOLE LIFE OF THE LANE.** `_perceiveCellFigures` restarted at index 0 every visit and counted ALREADY-HELD figures against its attempt bound of `cap * 4` = 24. Once the first 24 were banked, every later visit walked those 24, hit the bound and returned `perceived: 0` — **silently, because the log line only prints when something was perceived.** ⚠ The doc comment directly above it claimed *"they are taken in order, so a re-visited cell resumes past what it already holds"*. **It did not.** Cells hold far more than 24: 175 in the discrete-maths cell, **2,769** in one maths cell. **Harness on the real 175-figure list, 40 visits: 24/175 before → 175/175 after.**
  - ⛔⛔ **AND THE WORSE ONE — ACADEMIC VOCABULARY ANCHORING COULD BE COMPLETELY DEAD.** `_trainAcademicStories` pre-teaches unlearned content words as `newWords.slice(0, 60)`. `newWords` is rebuilt each visit by filtering out `_definitionTaughtWords` — which is **only ever added to when `defsBound > 0`**. **A word the dictionary has no entry for is never recorded**, so it stays unlearned, stays at the head in corpus order, and is retried every visit forever. Academic prose is full of them — proper nouns, inflected forms, technical coinages. **Once sixty collect at the head, the cell's remaining vocabulary can never be anchored again**, and the log still reads `0/60 anchored`, which looks like a bad batch rather than a wall. **Harness — 400 words, 60 undefinable at the head, 40 visits: 0 of 340 anchored before → 340 of 340 after.**
  - ⭐ **BOTH FIXED WITH A ROTATING CURSOR, AND A MISS-LIST WAS DELIBERATELY REJECTED.** Marking a word permanently-missed on a failed lookup cannot tell *"this word has no definition"* from *"the API refused me just now"* — the exact conflation that has bitten this corpus repeatedly. **A rotating window needs no such judgement:** a blocked head cannot hide the tail, and a word that becomes definable later is still picked up. An already-held figure also no longer counts as an attempt — it costs a store lookup, not a fetch.
  - ⛔ **A SECOND, QUIETER BUG IN THE SAME FUNCTION: the figure key was a POSITION.** `key: theme-${tried}` used the loop counter, so a re-ingest inserting one figure shifted every subsequent key by one and **silently re-bound every banked percept in that cell to a different picture** — undetectable, because the key still looked well-formed. Now keyed on the figure's own address via `figKeyOf()`: a URL identifies the picture, an index identifies where it happened to sit that day. Verified stable, distinct, and `src`-fallback-correct.
  - ✅ **THE SWEEP HE ASKED FOR — every bound on teaching material in the walk, classified, not just the two that were broken:**
  ```
    FIXED   _perceiveCellFigures        cap*4 attempt bound stranded the tail
    FIXED   _trainAcademicStories       slice(0,60) on an unclearing head
    OK      _teachSentenceList          no cap - trains every sentence
    OK      _teachVocabList             no cap - trains every word
    OK      life-curriculum accessors   no caps - return the whole cell
    OK      _phaseRepCursor             already persists a deferred phase's
                                        remainder through brain-weights.bin
    BY DESIGN  runCollege slice(0,100)  a GATE's measurement sample, not a
                                        teach - "Gate equation (register
                                        separation)". Left alone.
    BY DESIGN  _autoFinal slices        exam-question sampling, not material
  ```
  ⚠ **The two "BY DESIGN" rows are the point of doing this as a sweep rather than a search-and-replace** — a cap on a gate's sample is not the same defect as a cap on what she is taught, and "fixing" them would have changed what the gates measure.

- [x] `MAJORFEED.1` — ⭐⭐ **HER MAJOR NEARLY DOUBLED: 494,172 → 980,344 WORDS AND 895 FIGURES WHERE THERE WERE NONE.**
  - Gee (verbatim): *"okay welp get something that will help here beter learn her major im sure if u accutaly search far and wide and not just stoip for the first bit of eye candy you could find soimething"*
  - ⛔ **WHAT PROVOKED IT — her major was the worst-fed subject in the corpus**, against a 330,000-word college floor: theory 5-9%, systems 5-11%, **and zero figures in every one of those eight cells. A computer-science degree taught on twenty thousand words and no diagrams.**
  ```
                                          before      after   figures
    cstheory  college1 Discrete Maths     28,358 ->  86,253      173
              college2 Algorithms         16,880 ->  72,030        7
              college3 Theory of Comp     25,296     (none)        0
              college4 Advanced Algos     21,509 -> 104,626      224
    cssystems college1 Computer Org       20,696 ->  74,450       40
              college2 Comp Architecture  27,487     (none)        0
              college3 Operating Systems  17,838 ->  36,997       23
              college4 Networks           36,773 -> 237,348      247
    major     college1                    48,447 ->  64,969        6
  ```
  - ⭐ **"SEARCH FAR AND WIDE, NOT THE FIRST BIT OF EYE CANDY" WAS THE WHOLE JOB, AND ROUND ONE WOULD HAVE FAILED.** The first pass surfaced four books and **three were CC-BY-NC-ND** — Barak's *Introduction to Theoretical CS*, *Dive Into Systems*, Dordal's *Networks*. All excellent, all unusable, because **ND is not a judgement about the book — it is a statement about what this corpus does to it.** ⚠ **And the best book of the set showed NO licence on its own pages:** Peterson's licence lives in the repository's `LICENSE` file. **A probe that reads one page and concludes "no licence" rejects CC-BY 4.0 texts.**
  - **Wired, every licence read at source:** Peterson *Computer Networks: A Systems Approach* CC-BY 4.0 · *CS from the Bottom Up* CC-BY-SA · *Open Data Structures* CC-BY · Runestone *Problem Solving with Algorithms* CC-BY-NC-SA · Wikibooks *Operating System Design* CC-BY-SA · *Composing Programs* CC-BY-NC-SA · Levin *Discrete Mathematics* CC-BY-SA (**the same book placed a second time on purpose — `courseNameFor('cstheory','college1')` IS Discrete Mathematics**).
  - ⛔⛔ **MY OWN FETCHER MADE EXACTLY THE MISTAKE HE WARNED ABOUT, AND THE NUMBERS EXPOSED IT.** The first run reported **2,958 words for a whole networking textbook** and 4,183 for a data-structures textbook. Cause: **it read the table of contents and called it the book.** Those chapter pages are STUBS — Peterson's `foundation.html` holds 1,560 visible characters and links to eight sections below it. A two-level walk took Peterson **2,958 → 200,575** and Open Data Structures **4,183 → 55,150**. ⚠ **My first diagnosis was wrong too:** I suspected the sentence filters were over-rejecting and measured the attrition stage by stage before touching them — **18 of 18 candidate sentences survived every filter.** The pages simply had nothing on them. **Measuring first stopped me "fixing" a filter that was working.**
  - ⛔ **AND THE FIGURE COUNT WAS A LIE BEFORE IT WAS FILTERED: 5,693 "figures" from one book.** Looking at them: `next.png alt="next"`, `up.png`, `prev.png`, then hundreds of `img42.png alt="$ 10^6$"` — the site renders every inline LaTeX fragment as an image. **She would have banked a navigation arrow as a picture and a maths glyph as a diagram.** Two filters, both **structural, with no list of banned filenames** — a rendered formula is identified by the SHAPE of its alt text, and site furniture by REPETITION (an image on more than half a book's pages is chrome, whatever it is called). **5,693 → 7 honest figures**, and Peterson's 247 real network diagrams survive untouched.
  - ⏳ **TWO CELLS STILL HAVE NO SOURCE, and both are because the good book is ND:** `cstheory/college3` **Theory of Computation** (8%) and `cssystems/college2` **Computer Architecture** (8%). **Do not re-propose Barak or Dive Into Systems** — they were probed, they are right for the course, and the licence refuses this use.
