...
- [x] `FIELDSFLAG.1` — ⛔⛔ **THE CTL TIMEOUT SILENTLY CANCELLED SPONGE'S OWN FIX, AND THAT IS THE REAL BUG HERE.** `self-update.sh` now bounds `git lfs pull` at **45 m** (`UAL_LFS_TIMEOUT`). `brain-ctl.js` ran the same script under **`execFile(..., {timeout: 900000})` — 15 minutes.** ⛔ **On that path the parent is SIGTERMed half an hour before the guard could ever fire**, so the bound is unreachable and a deploy through ctl can never legally finish. **Two timeouts, two files, wrong order, each perfectly reasonable read alone.**
  ...
  - **16/16 exercised against the real lifted functions:** `45m → 65m` · `2h → 140m` · `0 → 45m floor` · `900` (bare seconds) · empty · unparseable → floor, never `NaN` · and an assertion that the old hardcoded `900000` was **below** the 45 m bound.
