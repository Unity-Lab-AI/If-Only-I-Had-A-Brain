...
- [ ] **DONORKILL.2** **the tooling gap this exposes: nothing outside the brain knows which GPU is load-bearing.** The RunPod pod list, the dashboard Clients table and the leaderboard all show a card without showing that it is THE primary holding her weights. Surface `isPrimary` (and the pause consequence) wherever a donor can be terminated. Same family as MIRRORID.5 / RESYNCDUTY.9 / SYNCPARTIAL.7 / LOOPNAME.7 - the board cannot answer "is it working?", and here it could not answer "what breaks if I kill this?"
