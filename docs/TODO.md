...
- [ ] `DEFPOS.2` — ⛔⛔ **2026-09-06 — I TRIED TO ANSWER THIS BY EXTENDING THE `OVERLAP` TABLE TO THE LIVE LOADS AND COULD NOT, AND THE REASON IS THE FINDING: THE SWEEP'S RESULTS WERE PRESERVED BUT ITS METHOD WAS NOT.** `rep-compression.js` carries `OVERLAP_LOADS`, `OVERLAP_K`, `OVERLAP_REPS`, `OVERLAP_RETRIEVAL`, `OVERLAP_REFERENCE` and `OVERLAP_VERDICT` — **every number and no harness.** My reconstruction returned **100.0% retrieval at every load from 0.264 to 37.3**, which is plainly wrong against the recorded `4.000 → 24.2%`, so the model is not the one that produced the table.
