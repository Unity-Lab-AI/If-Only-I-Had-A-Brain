...
| KI-4 | 🔴 OPEN | **Cognition / basin quality — mode collapse.** Trained basins collapse into superposition; motor argmax can lock on a single token. | `js/brain/curriculum.js`, cortex Hebbian path | Gee's domain (basin/cognition quality). `_rectifySemMotor()` rectify is armed to continue instead of halting on saturation. Deep dive: `docs/ISSUE-basin-collapse-fix.md`. |
