...
4. **Weights are float32 end to end, and the arithmetic is not.** Storage — GPU-resident and CPU master alike — is float32 as of 2026-09-05; the CPU master was float64 until then purely to be downcast at the socket, since the donor shaders bind `array<f32>` and WGSL has no float64 storage type at all. What remains mixed is deliberate: **summation stays float64** on the coordinator, because rounding error compounds across a fanout-300 row and not inside one stored weight. Measured at the real geometry, narrowing the storage costs a relative RMS error of 2.7e-8 (SNR 151.4 dB) and flips no spike thresholds in 20,000 neurons — below every noise source in the system, but not exact.
