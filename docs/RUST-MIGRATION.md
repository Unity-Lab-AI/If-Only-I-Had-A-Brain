...
| *"`student-question-banks.js` ← DATA"*, 2,323 lines | **Mixed.** It also exports `injectGeneratedExamQuestions`, `cutScoreFor`, `trainExamOverlap`, `examVocabCoverage`, `extractVocabFromBank`, `extractAcceptedVocabFromBank`, `NEVER_TEACH_EXAM_TOKENS`, `methodologyBankFor`, `scoreMethodologyAnswer` — that is logic, and `curriculum.js:43` imports those functions, not just the banks. ⛔ **And 2026-09-06 makes the "mixed" verdict load-bearing rather than tidy:** `extractVocabFromBank` decides *which words the curriculum will teach before an exam*, and getting that policy wrong had the pre-gate teaching the answers to the nonsense-word decoding probes. **A port that treats this file as data would carry the tables and drop the policy** — including `NEVER_TEACH_EXAM_TOKENS`, which is itself DERIVED from the banks at module load and was already shipped once as an empty set when a field-list mapper silently dropped the flag it reads |
