...
verified-scope: |
  CHECKED 2026-08-27 against source, mechanically enumerated and diffed:
    - the 12-page inventory vs `git ls-files '*.html'` (exact match) — ⚠ **run it, do not assume it**; this line said 11 for five days while the command returned 12
...
>
> **Updated 2026-09-07 (mind's-eye viewer):** inventory unchanged at 12 pages. `html/minds-eye.html` changed on four counts, and three of them were the page working against its own purpose. ⛔ **The frame carried a `rotateX(8deg) rotateY(-9deg)` tilt under a perspective stage, and a CSS transform does not change layout** — the element kept its upright box in flow while the browser painted it rotated, so the projected bottom edge landed **on top of the readout line below it**. Hover swapped in a *second* tilt rather than releasing. Both removed; hover now responds through colour, and the stage carries no `perspective` at all so the defect cannot come back with the next transform anyone adds. ⛔ **The fullscreen toggle deleted itself on entering fullscreen** (`btn.style.display = 'none'`) — and that line was not even the cause of its own disappearance: the button was a **sibling** of the fullscreen element, and only the fullscreen element's subtree is painted, so it was already invisible. Moved **inside** the stage, so it goes fullscreen with the picture and is a real switch in both directions. ⭐ **The canvas is now bounded by `max-height` as well as `max-width`** — a `max-*` bound never upscales, so the CRYSTAL 1:1 decision survives while a 1920×1467 field stops pushing the whole readout below the fold; the two prose blocks (1,335 and 2,374 characters) fold into `<details>` with **every word kept** for the same reason. ⛔⛔ **And the status line said `live` unconditionally while showing a six-hour-old frame.** Past five minutes it now reads **held**, with the reason taken from `state.voice` rather than guessed: the viewer is grounded-only by design, and while `innerVoiceHeld` is true with `wordsBucketed` at 0 she has no thought to aim an impression with, so nothing new qualifies and the last real picture correctly stays up. **The holding was right; claiming it was live was not.**
>
> ⛔⛔ **Corrected 2026-09-06 — THE INVENTORY IS 12 PAGES, NOT 11, AND HAS BEEN SINCE 2026-09-01.** `html/teachview.html` shipped on 2026-09-01 (`5cafe92b`) and was **never added to the table below**, so this doc has described itself as an *exact* match against the live tree for five days while being off by one. ⚠ **The check that would have caught it is written at the top of this very doc** — *"the 11-page inventory vs `git ls-files '*.html'` (exact match)"* — and `git ls-files '*.html'` returns **12**. **A verification procedure that is documented and never run is not a verification procedure.** ⭐ **`html/legend.html` was NOT missing it** (the page is carded there, and the drift check reads 10/10), so the page was reachable by anyone browsing; it was invisible only in the doc that claims to inventory everything — which is the harder failure to notice, because nothing breaks. Row added below. Every "11 pages" phrase in the dated entries beneath this one was true when written **except** the 2026-09-01-and-later ones, and they are left standing rather than rewritten.
>
...
| `html/teachview.html` | Admin training viewer — the exact text going into her at human reading speed, **beside her mind's eye**, because the words and the images are one education and not two. Paced chokepoint feed, complete per-cell ledger, per-lane / per-source bars, flags panel, knob panel, coverage panel. ⛔ **Every privileged route here carries the `/admin` prefix when deployed** — the public vhost forwards only five locations and everything else answers 200 with the SPA's HTML, which is the worst failure shape because every status check passes. ⭐ **2026-09-06 — its mind's eye is now the SAME reconstruction as `minds-eye.html`**, importing `reconstructImageData` from the mind-space transform rather than carrying a second implementation; the previous local copy read `rec.rgba`/`rec.pixels`, which no snapshot builder has ever written, so it had never drawn a frame. Also shows the server's `note`, source, equation count and frame age — the same facts the public page shows | Deployed: served static behind the admin lane. Local: `http://localhost:7525/html/teachview.html` | reads `/minds-eye.json` + `/public-state.json` publicly, all other routes via `/admin` | NO (admin) |
| `html/minds-eye.html` | Public "what Unity sees" viewer — polls one cached `GET /minds-eye.json` field C, reconstructs the image CLIENT-SIDE via the mind-space inverse CDF 9/7. Single shared source, no per-viewer compute. Sources: `seen-camera`/`seen:<word>` (live perception), `recall:<word>` (visual memory re-seen), `lookup:<word>` (a studied reference), **`canvas:own:<words>` (2026-08-20 — HER OWN drawing, constructed from a learned shape schema, never a filtered reference)**, `canvas:draw:<word>` (a render of what she LOOKED AT — kept, but no longer called a drawing), `canvas:imagine:<a>+<b>`, `thought`/`thought-blend`/`sem-state` (de-novo). **2026-08-20 also adds an "her art" stat line** (`drawn · shapes she can draw from · seen`) read from `/public-state.json` `state.ownArt` on a 15s poll, plus a source-label legend rewritten for the own-art distinction. **2026-09-07 — no tilt anywhere** (the transform was overhanging its own layout box and covering the readout), **canvas bounded by `max-height` as well as `max-width`** so the picture and its readout fit on screen together without ever being enlarged past 1:1, **a fullscreen toggle that lives inside the fullscreen element** and therefore stays visible as the switch back out, the two prose blocks folded into `<details>` with every word kept, and ⛔ **an honest `held` status past five minutes instead of an unconditional `live`** — a still frame is normally correct here, because the viewer is grounded-only and refuses to publish a mood texture. Linked from `index.html` 👁 MIND'S EYE footer button | Deployed + local: served static; same-origin poll + localhost dev fallback | reads `/minds-eye.json` + `/public-state.json` — the latter for `state.ownArt` **and `state.voice`** (the held reason). Every one degrades to "—" / "warming up" / the generic held wording, never inventing a number **or a cause** | YES (static + single-source poll) |
...
⭐ **All 12 pages are carded** (counted 2026-09-06 off `html/legend.html` itself: 14 cards — the 11 files in `html/` including legend's own, the root `index.html`, a download card, and one card that pairs `dashboard-public.html` with `dashboard.html?public=1`). ⚠ **This read "11" and the inventory table above also read "11", and they were counting DIFFERENT SETS** — which is why neither number caught `teachview.html` going unlisted for five days. The inventory is **every** page; the legend cards **every page plus a download link**. ⭐ **The legend was the honest one:** it has carded `teachview.html` since the day the page shipped. **The old text omitted `dashboard-public.html` and `minds-eye.html` entirely, which is how a page can exist in the inventory table above and be invisible in the narrative below it** — and then the same thing happened again, in the opposite direction. Section 3 points at `docs.html?doc=<slug>` plus a raw-`.md` fallback per doc, and lists **README + SETUP + ARCHITECTURE + EQUATIONS + ROADMAP + SKILL_TREE + SENSORY + WEBSOCKET only** — that part is still accurate. ⚠ **But read the docs-viewer section below before repeating "workflow + planning docs are not surfaced":** the legend's own lede scopes that claim correctly (*"not for public legend"*), and the viewer's whitelist has since grown to 19. **The legend is the advertisement, not the gate.**
