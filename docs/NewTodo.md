...
| P4.3 | [~] IN PROGRESS — Per-concern split underway. **P4.3.a** ✅ SHIPPED 2026-06-17 — 20 GPU sparse-comm methods extracted from brain-server.js (lines 2615-3687, 1073 lines) into new `server/brain-server/gpu.js` `SERVER_GPU_MIXIN`. Methods cover `_gpuStep`/`_gpuBatch` LIF dispatch + sparse-protocol comms + flow control + sparse-matrix ops + batched-hebbian queue + bound-projection ops + cortex sub-slice writes + readback. **CommonJS module pattern** (brain-server.js uses `require()`, not ESM `import`) — `module.exports = { SERVER_GPU_MIXIN }` on the module side, `const { SERVER_GPU_MIXIN } = require('./brain-server/gpu.js')` on the consumer side. Attached via `Object.assign(ServerBrain.prototype, SERVER_GPU_MIXIN)` at brain-server.js bottom. **P4.3.b state-broadcast** (pending — _broadcastStateNow, getState, _runDictionarySmokeTest, _recentBrainEvents). **P4.3.c memory** (pending — _initEpisodicDB, storeEpisode, _serializeEmbedding). **P4.3.d chat** (pending — processAndRespond + chat-Hebbian + multi-turn coherence). brain-server.js 9555 → 8506 lines after P4.3.a (−1049, −11%). README at `server/brain-server/README.md`. | `server/brain-server.js` + `server/brain-server/*.js` | [~] P4.3.a done · P4.3.b/c/d pending |
