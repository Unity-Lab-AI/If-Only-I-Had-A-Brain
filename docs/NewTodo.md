...
| # | Task | File:Line | Status |
|---|------|-----------|--------|
| P4.1 | **Split `curriculum.js` by GRADE — per-grade file, curriculum.js stays template architecture (operator 2026-06-17).** Each grade gets its own file under `js/brain/curriculum/<grade>.js` carrying that grade's specific teach methods + cell runners + content. `curriculum.js` stays as the ORCHESTRATOR / template architecture — common machinery (`_teachAssociationPairs`, `_teachHebbian`, `_teachConcreteSentences`, `_probeSentenceGeneration`, `runSubjectGrade`, cell-runner mechanics, gates, telemetry, helpers) lives there. Existing: `curriculum/pre-K.js` (511 lines), `curriculum/kindergarten.js` (7572 lines after P4.1.a — was 6430). Future: `curriculum/grade-1.js` through `curriculum/phd.js` created as each grade unlocks. Pre-K + K scope LAW still in force: only Pre-K + K files exist now; grade-1+ files created when their content is written. **PROGRESS:** P4.1.a SHIPPED 2026-06-17 (commit `7c0a2f3`) — 13 K-ELA letter/phoneme/word teach helpers extracted from curriculum.js lines 6774-7905 into kindergarten.js K_MIXIN. P4.1.b SHIPPED 2026-06-17 (commit `0c95cb5`) — 5 K-only direct-Oja letter/word teach helpers extracted from curriculum.js lines 6238-6772 into kindergarten.js K_MIXIN. P4.1.c SHIPPED 2026-06-17 — 3 orphan Session-25 legacy methods `_teachAlphabetSequence/_teachLetterNames/_teachLetterSounds` migrated from curriculum.js lines 6082-6176 into kindergarten.js K_MIXIN with DEPRECATED-LEGACY header + stale section-header chrome + fragmented P4.1.a/b markers CONSOLIDATED into single coherent extraction-reference block. Cumulative: 21 methods, 1800 lines moved. curriculum.js 26033 → 24250 lines (−1783, −6.8%). kindergarten.js 6430 → 8260 lines (+1830, +28.5%). P4.1.d PENDING — audit + migrate `_teachDigitSequence/_teachDigitNames/_teachMagnitudes/_teachCVCReading/_teachSightWords` (now at lines ~7600-7800 after cumulative shrink, ~213 lines, caller status TBD via deeper grep audit). | `js/brain/curriculum.js` (extract per-grade content), `js/brain/curriculum/kindergarten.js` (receive K-grade migrations), `js/brain/curriculum/pre-K.js` (audit content) | [~] P4.1.a + P4.1.b + P4.1.c done · P4.1.d pending |
