...
| P4.2 | [~] IN PROGRESS — Per-module split started. **P4.1.a-pattern bites planned:** P4.2.a telemetry module (✅ SHIPPED 2026-06-17 — 6 methods, 215 lines extracted to `js/brain/cluster/telemetry.js` `CLUSTER_TELEMETRY_MIXIN` — trackRecentEmission, initCompositionalTelemetry, classifyCompositionalEmission, _recordWordCreationCandidate, getWordCreationCandidates, getCompositionalStats; attached via Object.assign at cluster.js entry-point bottom). P4.2.b emit (pending — composeSentence + emitWordDirect + generateSentenceAwait + _emitDirectPropagate + _dictionaryOracleEmit, ~2000 lines). P4.2.c hebbian (pending — _crossRegionHebbian + intraSynapsesHebbian + intraSynapsesBcm + anti-Hebbian variants, ~800 lines). P4.2.d probe (pending — computePhi + getTrainedCapability + diagnoseReadoutForEmbedding + workingMemoryReadout, ~400 lines). cluster.js 6375 → 6179 lines after P4.2.a. README at `js/brain/cluster/README.md`. | `js/brain/cluster.js` + `js/brain/cluster/*.js` (per-module split) | [~] P4.2.a done · P4.2.b/c/d pending |
