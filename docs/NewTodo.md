...
| # | Task | File:Line | Status |
|---|------|-----------|--------|
| P4.1 | **Split `curriculum.js` by GRADE — per-grade file, curriculum.js stays template architecture (operator 2026-06-17).** Each grade gets its own file under `js/brain/curriculum/<grade>.js` carrying that grade's specific teach methods + cell runners + content. `curriculum.js` stays as the ORCHESTRATOR / template architecture — common machinery (`_teachAssociationPairs`, `_teachHebbian`, `_teachConcreteSentences`, `_probeSentenceGeneration`, `runSubjectGrade`, cell-runner mechanics, gates, telemetry, helpers) lives there. Existing: `curriculum/pre-K.js` (511 lines), `curriculum/kindergarten.js` (7572 lines after P4.1.a — was 6430). Future: `curriculum/grade-1.js` through `curriculum/phd.js` created as each grade unlocks. Pre-K + K scope LAW still in force: only Pre-K + K files exist now; grade-1+ files created when their content is written. **PROGRESS:** P4.1.a SHIPPED 2026-06-17 (commit `7c0a2f3`) — 13 K-ELA letter/phoneme/word teach helpers extracted from curriculum.js lines 6774-7905 into kindergarten.js K_MIXIN (`_teachLetterCaseBinding/_teachLetterNaming/_teachVowelSoundVariants/_teachWordEmission/_teachRhymeFamilies/_teachSyllableCounts/_teachCVCSoundIsolation/_teachPluralTransform/_teachQuestionWordCategories/_teachEndPunctuation/_teachStoryComprehension/_teachPhonemeBlending/_teachCapitalization`). P4.1.b SHIPPED 2026-06-17 — 5 K-only direct-Oja letter/word teach helpers extracted from curriculum.js lines 6238-6772 into kindergarten.js K_MIXIN (`_teachLetterSequenceDirect/_teachWordSpellingDirect/_teachLetterNamingDirect/_teachWordEmissionDirect/_teachWordSpellingDirectFinal`). Cumulative: 18 methods, 1705 lines moved. curriculum.js 26033 → 24349 lines (−1684, −6.5%). kindergarten.js 6430 → 8154 lines (+1724, +26.8%). P4.1.c PENDING — delete or migrate orphan methods `_teachAlphabetSequence/_teachLetterNames/_teachLetterSounds` (lines ~6082-6177, no active callers, Session 25 legacy superseded) + cleanup orphan doc-block at 6194-6199. P4.1.d PENDING — audit + migrate `_teachDigitSequence/_teachDigitNames/_teachMagnitudes/_teachCVCReading/_teachSightWords` (lines now ~7700-7900 after P4.1.a+b shrink, ~213 lines, caller status TBD). | `js/brain/curriculum.js` (extract per-grade content), `js/brain/curriculum/kindergarten.js` (receive K-grade migrations), `js/brain/curriculum/pre-K.js` (audit content) | [~] P4.1.a + P4.1.b done · P4.1.c/d pending |
