...
| P4.2 | [~] IN PROGRESS — Per-module split underway. **P4.2.a** ✅ SHIPPED 2026-06-17 (6 telemetry methods, 215 lines, `cluster/telemetry.js`). **P4.2.c** ✅ SHIPPED 2026-06-17 (6 Hebbian + GPU-upload methods — `_crossRegionHebbian` with P2.3 kScales plumbing preserved + `initGpu` + `intraSynapsesHebbian` + `intraSynapsesBcm` + `_crossRegionAntiHebbian` + `intraSynapsesAntiHebbian`, 691 lines, `cluster/hebbian.js` `CLUSTER_HEBBIAN_MIXIN`). cluster.js 6375 → 5500 lines after P4.2.a + P4.2.c (−875 net, −13.7%). **P4.2.b** (pending — composeSentence + emitWordDirect + generateSentenceAwait + _emitDirectPropagate + _dictionaryOracleEmit + generateSentence, ~2000 lines, largest remaining sub-bite). **P4.2.d** (pending — computePhi + getTrainedCapability + diagnoseReadoutForEmbedding + workingMemoryReadout + synapseStats, ~400 lines). README at `js/brain/cluster/README.md`. All migrations via deterministic Node scripts in `.git/`. | `js/brain/cluster.js` + `js/brain/cluster/*.js` | [~] P4.2.a + P4.2.c done · P4.2.b + P4.2.d pending |
