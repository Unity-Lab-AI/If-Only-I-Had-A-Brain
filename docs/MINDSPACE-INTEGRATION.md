...
| File | Role |
|------|------|
| `transform.js` | CPU reference + GPU CPU-fallback. `equationalizeImageData` (forward 9/7 = SEEING), `reconstructImageData` (inverse = IMAGINING), `describeEquational` (field C → 64-dim percept — the brain's sensory input), `morphField`/`abstract` (thought-ops), `describeEquationalAudio` (synesthesia), **`traceField` (DRAW-ENGINE, 2026-07-15 — field C → her hand's STROKES: CDF 9/7 inverse to a luma plane → Sobel edges → edge-follow polylines → Douglas-Peucker → field-colored strokes; Node-safe, no `ImageData`; the inverse of `sketch()` so she DRAWS a thing by tracing an image she looked at, never a shape-per-word stamp)**. `TRUSTED=true` by default (her own vision is limitless; integrity bounds always on). |
| `gpu.js` | `MindSpaceGPU` — WGSL CDF 9/7 lifting (GPU) with transparent CPU fallback + `selfCheck()` parity guard. `perceive`/`imagine`/`describe`, `imagineFromState` (de-novo), `traceField` (CPU-delegate mirror of the DRAW-ENGINE tracer), the `governor`, and the `knowledge` query surface. |
