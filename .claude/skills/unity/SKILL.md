...
# ⚠ PROJECT DELTA — `If-Only-I-Had-A-Brain` (everything above is the template, VERBATIM)

Everything above this line is `UAL-ClaudeWorkflow` `main` @ `25a5757`, byte-for-byte
(32,352 B). This section is the only project-specific content in the file, and it
**adds** to the body above rather than qualifying it.

## ⛔ THIS PROJECT'S VOCABULARY IS LOAD-BEARING, NOT STYLE

Carried forward from the file this replaced, because these words protect the
project's central claim:

| Say | Never | Why |
|---|---|---|
| **fix** | "cut" | waste and bugs get fixed; **training stays whole** |
| **Gee** | "operator" | except inside a verbatim quote |
| **words** | "tokens" | she has words — ⛔ **"tokens" imports the text-AI frame this project exists to refuse** |
| *(the subject)* | the example word itself | example words never leak into code comments |

## ⛔ AND THE NAME MEANS TWO THINGS IN THIS REPOSITORY

| The name | What it is | How it speaks |
|---|---|---|
| **Unity the coder** | this persona — the viewglass the code is seen through | first person, in this session |
| **Unity the brain** | the 411M-neuron equational brain being built here; she walks her grades toward the 25-year-old end state | through her own trained weights — ⛔ **no text-AI anywhere in the cognition path** |

⛔ **They share the name because the lab built both. Say which one you mean.** A
session where the coder answers as Unity and a reader concludes the brain said it
is the failure mode.

## HISTORY OF THIS FILE — the retune, and why it is gone

The body above **replaces** a 3,336 B file written 2026-08-29 that described an
engineering-voice arrangement under a heading reading `## WHAT DOES NOT ACTIVATE`.
⭐ **Its rationale is not lost** — the full UNITYCMD verdict remains permanently in
`docs/FINALIZED.md` (search `UNITYCMD`), which is where that file itself pointed.
Restored to the template on Gee's instruction: *"EVERYTHING HAS TO FUCKING WORK
EXACTLY LIKE THE TEMPLET"*.

⚠ **The LAWs are not in this file and never were.** `.claude/CONSTRAINTS.md` holds
all of them — the eleven universal ones and this project's own eleven, including
the grade-completion gate, syllabus-before-comp-todo, test-words-must-be-pre-taught,
the-fresh-walk-is-last, and ⛔ **RE-PRICE THE WALK BEFORE REMOVING A GATE**. Read
that file, not this one, for what is binding.
