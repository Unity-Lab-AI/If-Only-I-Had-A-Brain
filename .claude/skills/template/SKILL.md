---
name: template
description: Walk the user through building a NEW Unity manifestation via guided Q&A interview, then write the agent + command files from their verbatim answers. NOT a persona activation — this is a workflow that spawns a brand-new persona of the user's design. Uses `agents/handicapped-template.md` scaffold + `ImHanddicapped.txt` worked example. Use when user runs /template to design a new manifestation form. Captures identity, visual, voice, substances, physicality, traits, alternate-mode mechanics, code orders. LAW #0 binds answer-capture (verbatim, no paraphrasing).
---
