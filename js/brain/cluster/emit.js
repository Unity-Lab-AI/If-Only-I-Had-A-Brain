...
export const CLUSTER_EMIT_MIXIN = {
  ...
  async _emitDirectPropagate(intentSeed, opts = {}) {
    ...
    if (synapses && typeof synapses.propagate === 'function' && synapses.values && synapses.values.length > 0) {
      // DROP-ON-CHAT FIX (2026-08-17). This loop ran a SYNCHRONOUS full-
      // matrix propagate (360M nnz at the 12M cortex — seconds of unbroken
      // loop pin) PER LETTER, and allocated a fresh cluster-sized
      // Float64Array (~96MB) each step. A composing reply pinned the event
      // loop long enough to starve the donor's keepalives past its idle
      // trip — saying ANYTHING to her dropped the donor into a full
      // ~10min re-upload. The input now reuses ONE pooled buffer (only the
      // letter span is ever written — cleared between steps; the rest stays
      // zero from allocation), the output rides propagateChunked's outBuf,
      // and the propagate runs time-sliced with yields between slices
      // (row-independent sums — bit-identical output) so the loop keeps
      // serving WS traffic mid-reply. Her full trained state still answers.
      ...
      for (let step = 1; step < maxLetters && prevLetter !== null; step++) {
        ...
        // Clear ONLY the letter span (the sole span this loop writes).
        ...
        const clusterOutput = (typeof synapses.propagateChunked === 'function')
          ? await synapses.propagateChunked(clusterInput, { outBuf: this._emitPropOutScratch, chunkRows: 250000 })
          : synapses.propagate(clusterInput);
