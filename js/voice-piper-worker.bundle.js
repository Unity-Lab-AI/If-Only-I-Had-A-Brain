...
var PIPER_DATA = "/voice-engine/piper/piper_phonemize.data";
var ORT_WASM_DIR = "/voice-engine/ort/";
var OPFS_DIR = "unity-voice";
var OPFS_MODEL = "en_US-hfc_female-medium.onnx";
Z.wasm.wasmPaths = ORT_WASM_DIR;
Z.wasm.numThreads = 1;
Z.allowLocalModels = false;
var _session = null;
var _config = null;
async function opfsRead(name2) {
  try {
...
async function opfsWrite(name2, blob) {
  try {
    const root = await navigator.storage.getDirectory();
    const dir = await root.getDirectoryHandle(OPFS_DIR, { create: true });
    const fh = await dir.getFileHandle(name2, { create: true });
...
function phonemize(text) {
  return new Promise((resolve, reject) => {
    createPiperPhonemize({
      ...
      locateFile: (url) => url.endsWith(".wasm") ? PIPER_WASM : url.endsWith(".data") ? PIPER_DATA : url
    }).then((module2) => {
      module2.callMain([
        "-l",
        _config.espeak.voice,
        "--input",
...
async function synth(text) {
  ...
  const feeds = {
    input: new Me("int64", BigInt64Array.from(ids, (v) => BigInt(v)), [1, ids.length]),
    input_lengths: new Me("int64", BigInt64Array.from([BigInt(ids.length)])),
    scales: new Me("float32", Float32Array.from([
...
self.onmessage = async (e) => {
  ...
  try {
    if (type === "preload") {
      ...
      self.postMessage({ type: "pcm", id: id2, pcm: copy, sampleRate }, [copy.buffer]);
    }
  } catch (err) {
    self.postMessage({ type: "error", id: id2, error: String(err && err.message || err) });
  }
};
