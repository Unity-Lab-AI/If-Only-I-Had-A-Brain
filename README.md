...
| `scripts/` | Build + dev tooling | `stamp-version.mjs` (BUILD stamp on commit), `vox-build-bank.mjs` (voice bank), `unity-chat-hold.mjs` + `unity-say-live.mjs` (live-chat harness), `gatling-savestart.js`. ⛔ **Scripts that edit code, files or the stack are banned** — Edit/Write only, and any genuinely-necessary one-shot is deleted in the same commit that used it |
| `assets/social/` | Per-page social cards | One 1200×630 `og:image` per page (no shared card, no collage). ⚠ **The generator is gone** — `scripts/social-shots.mjs` was removed in the script purge, so `npm run social:shots` no longer exists and the current cards date from 2026-06-26. Re-shooting them needs the generator rebuilt first |
...
- `assets/README.md` — per-page social-image system (one `og:image` + custom description per page). ⚠ Its regeneration instructions describe `npm run social:shots`, which **no longer exists**
