...
| `crates/` | Rust crates shared with the donor. `unity-protocol` holds the donor wire contract — SPRS frames, opcodes and message types — which previously existed twice, once in Rust and once in JS, kept in step by hand. A Cargo workspace at the repo root makes `donor-app/` depend on it by path instead of by copy | Not a mixin — a Cargo workspace member |
