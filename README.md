...
| `scripts/` | Build + dev tooling | `stamp-version.mjs` (BUILD stamp on commit), `social-shots.mjs` (per-page social-card generator — Playwright), `unity-chat-hold.mjs` + `unity-say-live.mjs` (live-chat harness), `gatling-savestart.js`, `gatling-ctl.js` (the control-plane twin — fires at `/ctl/*`, reads before it acts, refuses to cycle a young process; for the brain that is alive and not answering, where the brain-route gatling can only 504). ⛔ **Scripts that edit code, files or the stack are banned** — Edit/Write only, and any genuinely-necessary one-shot is deleted in the same commit that used it. Repeatable BUILD tools that emit artifacts are the exception, and are the only things that live here |
