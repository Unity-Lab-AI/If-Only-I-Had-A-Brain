...
const KNOBS = [
  ...
  },
  {
    ...
    why: 'WAS 60, HARDCODED. Set to unlimited on the operator\'s ruling. The arithmetic for keeping it was mine and wrong twice: 1,996,943 words was distinct-PER-CELL summed, when _definitionTaughtWords is global and persisted so the real figure is 365,132 distinct across the whole corpus (5.5x overcount); and those were priced as cold SERIAL lookups when prefetchDefinitions batches them (another 20x). Real cost of no cap was published as 19.8h across a ~24-day walk. CORRECTED 2026-09-06: that price is UNDERSTATED twice over and the decision is unchanged. PREFETCH_CONCURRENCY is 5, not the 20 this text claimed — it was lowered deliberately because dictionaryapi.dev rate-limits hard, and the arithmetic was never redone against it; and it priced the PREFETCH only, never the serial per-word anchoring loop after it, which is where the Hebbian binding happens. Measured live on ela/kindergarten: 17,873 distinct content words, 70.4% answered offline in 1.5s, 5,288 to the network = 1,058 sequential chunks, observed frozen for 33+ minutes. The cap still stays off — the ruling is about what she can ever know and a truer price does not change it; what the bad price hid was that the region reported NOTHING, now fixed with per-chunk stamps.',
    effect: 'live', site: 'js/brain/curriculum.js — _trainAcademicStories pre-vocab block',
    proof: 'read inside the method on every cell',
    what: '⚠ DIAGNOSTIC LEVER ONLY — for bisecting a slow cell, never an operating setting. Caps how many unlearned words a cell looks up definitions for before its prose is taught. ⛔ It was hardcoded at 60 and the cap came off 2026-09-02 on the operator\'s ruling ("she has to be able to look up all workds she needs to know no some bullshit limit"). The arithmetic that appeared to justify keeping it was wrong twice: distinct-per-cell words were summed (1,996,943) when the figure that matters is distinct across the whole corpus (365,132, because _definitionTaughtWords is global and persisted), and those were then priced as cold serial lookups while prefetchDefinitions batches them. ⛔ AND THE REPLACEMENT PRICE WAS ALSO WRONG — corrected 2026-09-06: PREFETCH_CONCURRENCY is 5 (not the 20 quoted), and the 19.8h figure covered the prefetch alone and omitted the serial anchoring loop entirely. On ela/kindergarten the prefetch alone is 1,058 sequential chunks and was observed frozen for 33+ minutes with no progress reported. ⭐ The cap still stays off, and this is the point: a cap here was never a cost control, it was a ceiling on what she can ever know. The real defect was silence, not duration — the pass now stamps prevocab:<subject>-<grade>:N/M every chunk.',
  },
  {
