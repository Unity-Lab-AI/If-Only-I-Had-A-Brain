...
> 3. ⛔⛔ **THE EXCLUDE LIST THAT RUNS IS THE BOX'S, NOT `main`'s** (found 2026-09-04). `brain-server.js` resolves the script from `__dirname`, so a press executes **whatever is already installed** — the version on `main` only takes effect on the press *after* the one that delivers it. **That makes point 1 a contract between two commits, and the older one is the enforcer.** The box was found at a commit predating ONEREPO, whose list had no `--exclude 'corpora'`; since ONEREPO untracked all 227 corpus files from this repo, that press would have `--delete`d the books **and the embedding table** off the box and restarted into a boot that stops by design. **Before any press: read `state.build.short`, then `git show <that>:deploy/self-update.sh` and diff it against `main`'s. When they differ, the press is a TWO-PRESS sequence** — the first delivers the new script, the second runs it and repairs what the old list did not know to protect. `unity-brain-ctl` is a separate always-up service, so `/ctl/update` fires the second press with the brain down.
>
