...
#
# ⚠⛔ CORRECTED 2026-09-06 — THIS SENTENCE WAS FALSE AND IT DESCRIBED A SAFETY
# THAT DOES NOT EXIST. It read: "`fields` AND `corpora` are both on the rsync
# --delete EXCLUDE list above, or the next press would delete everything this
# step just downloaded."
#   · `fields` IS on the list. That half was true.
#   · `corpora` IS NOT. The only corpora entry is `corpora/glove.6B.*`.
# `CORPORA_DIR` defaults to `$BACKEND_DIR/corpora`, i.e. INSIDE the overlay's
# `--delete` destination, and the code repo ships only `corpora/vocabulary/`
# (the books are gitignored and live in the data repo) — so the overlay does
# delete the books, and what actually restores them is THIS step running
# afterwards, not an exclude. The books survive because of ORDER, not because of
# protection, and a comment claiming protection is how the next reader stops
# checking the order.
# ⚠ The real consequence of the missing exclude is that EVERY press re-downloads
# the entire book corpus, and a press whose clone fails leaves the box with the
# books already deleted — which is the asymmetry the block below calls fatal.
#
...
if [ "${UAL_SKIP_FIELDS:-0}" = "1" ]; then
  ...
  if _clone_out="$(env GIT_LFS_SKIP_SMUDGE=1 ${_git_env} git clone ${_clone_extra} --branch main "$DATA_REMOTE" "$FTMP/bw" 2>&1)"; then
    ...
    # ⛔⛔ `--exclude 'vocabulary'` IS LOAD-BEARING AND IT IS NEW (2026-09-06).
    #
    # This is a `--delete` MIRROR of the DATA repo onto $CORPORA_DIR, and it runs
    # AFTER the code overlay. `corpora/vocabulary/*.json` is owned by the CODE
    # repo — 20 tracked files, generated by `gen-grade-vocab.mjs`, shipped by the
    # overlay a few hundred lines above. Without this exclude the mirror deletes
    # them the instant the data repo does not happen to carry a `vocabulary/` of
    # its own.
    #
    # ⛔ THE CONSEQUENCE IS NOT A DEGRADED WALK, IT IS A DEAD ONE.
    # `gradeVocabularyFor()` THROWS on a missing list rather than returning `[]`
    # — deliberately, because "a grade that teaches zero words looks complete and
    # deposits nothing". That throw lands inside the curriculum, and this
    # codebase proved once this week that a caught init-time throw produces a
    # brain that serves state, attaches a donor and reads healthy on every
    # dashboard field while teaching nothing at all.
    #
    # ⚠ FOUND BY READING, NOT BY BREAKING. The comment ~600 lines above claims
    # "`fields` AND `corpora` are both on the rsync --delete EXCLUDE list" — and
    # `corpora` is NOT on it; only `corpora/glove.6B.*` is. That false sentence
    # is corrected at its own site. Whether the data repo currently carries a
    # `vocabulary/` is unknown from here and DOES NOT MATTER: this exclude makes
    # the outcome identical either way, which is exactly the point.
    if [ -d "$FTMP/bw/corpora" ] && rsync -a --delete --exclude 'vocabulary' "$FTMP/bw/corpora/" "$CORPORA_DIR/" >> "$LOG" 2>&1; then
